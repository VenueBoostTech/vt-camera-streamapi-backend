...
def parse_arguments():
    """Parse command line arguments for face detection model training"""
    parser = argparse.ArgumentParser(description='Train face detection model')
    parser.add_argument('--data_path', type=str, 
                        default='training/data/Face Detection.v1i.yolov11/data.yaml',
...
def setup_paths(data_path):
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))  # face_detection directory
...
def train_model(model, dataset_path, args, paths):
    ...
    print("Starting face detection model training...")
    
    # Configure training parameters - optimized for face detection
    results = model.train(
        data=dataset_path,
        epochs=args.epochs,
        imgsz=args.img_size,
        ...
        cos_lr=True,
        augment=True,
        mixup=0.0,  # Less mixup for faces
        flipud=0.0,  # No vertical flips for faces
        fliplr=0.5,  # Horizontal flips are good for faces
        mosaic=0.8,  # Slightly reduce mosaic for faces
        degrees=5.0,  # Less rotation for faces
        translate=0.1,
        scale=0.2,   # Less scale variation for faces
        shear=0.0,   # No shear for faces
        perspective=0.0,  # No perspective change for faces
        hsv_h=0.015,     # Slight hue variation
        hsv_s=0.4,       # Saturation variation
        hsv_v=0.4,       # Value variation
        multi_scale=True,
        rect=False,       # No rectangular training for faces
        resume=False
    )
...
def save_model(paths):
    ...
    best_model_path = os.path.join(paths['run_dir'], 'face_detection_model/weights/best.pt')
    
    if os.path.exists(best_model_path):
        face_detection_model_path = os.path.join(paths['models_dir'], 'face_detector.pt')
        shutil.copy(best_model_path, face_detection_model_path)
        print(f"Model saved to central models directory: {face_detection_model_path}")
        return face_detection_model_path
    else:
...
def main():
    """Main function to train face detection model"""
    # Parse arguments
    ...
    print(f"Training completed. Model saved to {os.path.join(paths['run_dir'], 'face_detection_model')}")
