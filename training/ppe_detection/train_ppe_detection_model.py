...
def parse_arguments():
    """Parse command line arguments for PPE detection model training"""
    parser = argparse.ArgumentParser(description='Train PPE detection model')
    parser.add_argument('--data_path', type=str, 
                        default='training/data/safety.v1i.yolov11/data.yaml',
...
def setup_paths(data_path):
    ...
    current_dir = os.path.dirname(os.path.abspath(__file__))  # ppe_detection directory
...
def train_model(model, dataset_path, args, paths):
    ...
    # Configure training parameters - optimized for PPE detection
    results = model.train(
        data=dataset_path,
        epochs=args.epochs,
        imgsz=args.img_size,
        ...
        cos_lr=True,
        augment=True,
        mixup=0.1,  # Some mixup for PPE detection
        flipud=0.1,  # Some vertical flips
        fliplr=0.5,  # Horizontal flips
        mosaic=1.0,  # Full mosaic augmentation
        degrees=10.0,  # More rotation for various PPE orientations
        translate=0.1,
        scale=0.5,   # More scale variation for PPE of different sizes
        shear=2.0,   # Some shear augmentation
        perspective=0.0,  # No perspective change
        hsv_h=0.015,     # Slight hue variation
        hsv_s=0.7,       # More saturation variation for various PPE colors
...
def save_model(paths):
    ...
    best_model_path = os.path.join(paths['run_dir'], 'ppe_detection_model/weights/best.pt')
    
    if os.path.exists(best_model_path):
        ppe_detection_model_path = os.path.join(paths['models_dir'], 'ppe_detector.pt')
        shutil.copy(best_model_path, ppe_detection_model_path)
        print(f"Model saved to central models directory: {ppe_detection_model_path}")
        return ppe_detection_model_path
    else:
...
def main():
    """Main function to train PPE detection model"""
    # Parse arguments
    ...
    print(f"Training completed. Model saved to {os.path.join(paths['run_dir'], 'ppe_detection_model')}")
